...
static int
ambitv_v4l2_grab_close_device(struct v4l2_grab* grabber)
{
   int ret = 0;
...
static void
ambitv_v4l2_grab_free_buffers(struct v4l2_grab* grabber)
{
...
void
ambitv_v4l2_grab_free(struct ambitv_source_component* component)
{
   struct v4l2_grab* grab_priv = (struct v4l2_grab*)component->priv;
...
ambitv_v4l2_grab_create(const char* name, int argc, char** argv)
{
   struct ambitv_source_component* grabber =
      ambitv_source_component_create(name);
...
errReturn:
   ambitv_source_component_free(grabber);
